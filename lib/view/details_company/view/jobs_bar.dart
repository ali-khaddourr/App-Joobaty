...
class JobsBar extends StatefulWidget {
  final int? companyId;

  const JobsBar({super.key, required this.companyId});

  @override
  _JobsBarState createState() => _JobsBarState();
}

class _JobsBarState extends State<JobsBar> {
  final controller = Get.put(DetailsCompanyController());
  ...
  void initState() {
    ...
    if (widget.companyId != null) {
      ...
      controller.fetchData(companyId: [widget.companyId!]);
      print(controller.dataJobs);
    }
  }

  @override
  Widget build(BuildContext context) {
    return GetBuilder<DetailsCompanyController>(
      builder: (controller) {
        return controller.isLoadingJob
            ? Center(
          child: Column(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              final job = controller.dataJobs[index] ?? {};
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            TextCustom(
                              text: job["company_name"] ?? "null",
                              size: MySize.fontSizeMd,
                            ),
                            SizedBox(height: MySize.sm),
                            TextCustom(
                              text: job["job_type_name"] ?? "null",
                              size: MySize.fontSizeMd,
                            ),
                            SizedBox(height: MySize.sm),
                            Container(
